...
const Navbar = () => {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 transition-all duration-500 border-b border-white/20 
      ${isScrolled
    ...
    >
          ...
          <div className="flex items-center gap-6 relative">
            ...
            {showCategories && (
              <>
                ...
                />
                <div className="absolute top-full mt-3 left-0 w-72 z-50 bg-white rounded-[2.5rem] shadow-2xl border border-slate-50 p-3 animate-in fade-in zoom-in-95 duration-300">
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`lg:hidden fixed inset-0 z-[100] transition-opacity duration-500 ${
          mobileMenu ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'
        }`}
      >
        {/* Backdrop */}
        <div className="absolute inset-0 bg-black/20 backdrop-blur-2xl" onClick={() => setMobileMenu(false)} />

        {/* Slide-in Panel */}
        <div
          className={`absolute top-0 right-0 w-4/5 max-w-md h-full bg-white shadow-xl flex flex-col p-8 pt-24 transition-transform duration-500 ${
        ...
        >
          ...
          <button
            ...
            className="absolute top-6 right-6 w-12 h-12 flex items-center justify-center bg-slate-900 text-white rounded-2xl shadow-md hover:scale-105 transition-transform"
          >
              ...
              {['Home', 'Offers', 'Track My Order', 'Contact'].map((item, idx) => (
                <li
                  ...
                  className={`transition-all duration-500 ${
                    mobileMenu ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'
                  }`}
                >
                  <Link
                    to={getPath(item)}
