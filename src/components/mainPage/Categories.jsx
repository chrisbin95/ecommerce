...
const Categories = () => {
  ...
  return (
    <div className="category-container bg-white min-w-[240px] py-2">
      {data.map((value, index) => (
        <Link 
          to={`/category/${value.cateName.toLowerCase().replace(/ & /g, "-")}`} 
          key={index}
          className="group flex items-center px-4 py-3 hover:bg-orange-50 transition-all duration-200 border-b border-gray-50 last:border-0"
        >
          <div className="w-10 h-10 flex items-center justify-center bg-gray-50 rounded-lg group-hover:bg-white transition-colors shadow-sm">
            <img 
              src={value.cateImg} 
              alt={value.cateName} 
          ...
          <span className="ml-4 text-sm font-medium text-gray-700 group-hover:text-orange-600">
            {value.cateName}
          </span>
