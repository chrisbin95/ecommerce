...
const Dcard = () => {
      ...
      try {
        ...
        for (const productDoc of productsSnapshot.docs) {
          for (const subName of subNames) {
            ...
            const productSnap = await getDocs(query(subRef, limit(10)));
            const list = await Promise.all(
              productSnap.docs.map(async (doc) => {
                const data = doc.data();
                try {
  ...
  if (error) return <div className="h-40 flex items-center justify-center font-mono text-xs text-red-400">{error}</div>;
